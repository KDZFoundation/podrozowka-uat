...
type ManifestOperation = "freeze" | "get_header" | "get_chunk" | "get_header_by_id" | "get_chunk_by_id";
...
const parseRequest = async (request: Request) => {
  ...
  const printJobIds = Array.isArray(body?.print_job_ids)
    ...
    : [];
  const operation: ManifestOperation = body?.operation === "get_header_by_id" || body?.operation === "get_chunk_by_id"
    ? body.operation
  ...
  if (operation === "get_header_by_id" || operation === "get_chunk_by_id") {
    if (!manifestId) throw new Error("manifest_id_required");
  } else {
  ...
  if ((operation === "get_chunk" || operation === "get_chunk_by_id") && (!Number.isInteger(chunkIndex) || Number(chunkIndex) < 0)) {
    throw new Error("chunk_index_required");
  }
  return { batchId, batchRevision, printJobIds, manifestId, operation, chunkIndex };
};
...
export const createPodPrintManifestHandler = (dependencies: PodPrintManifestRouteDependencies) => ({
  fetch: async (request: Request) => {
    ...
    try {
      ...
      if (input.operation === "get_header_by_id" || input.operation === "get_chunk_by_id") {
        const header = await dependencies.store.readHeader(input.manifestId);
        if (!header || header.data.state !== "frozen") return json({ error: "manifest_not_found" }, 404);
        if (input.operation === "get_header_by_id") return json({ header: header.data });
        const chunkId = `${input.manifestId}-${String(input.chunkIndex).padStart(6, "0")}`;
        const chunk = await dependencies.store.readChunk(chunkId);
        if (!chunk) return json({ error: `manifest_chunk_missing:${chunkId}` }, 422);
