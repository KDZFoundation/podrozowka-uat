...
try {
  ...
  foreach ($functionName in @('p24-webhook', 'hotpay-webhook', 'create-payment', 'inpost-shipx-webhook', 'inpost-geowidget-config', 'orlen-widget-config', 'contact-form', 'pod-production-batches', 'issue-fiscal-document', 'fiscal-document-pdf')) {
  ...
  foreach ($functionName in @('admin-payment-status', 'admin-inpost-settings', 'admin-orlen-settings', 'create-inpost-shipment', 'buy-inpost-shipment', 'get-inpost-label', 'sync-firmino-article')) {
