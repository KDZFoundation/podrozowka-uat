-- ORLEN Paczka configuration is held server-side in shipping_settings.
-- The map token is intentionally separate: it is a browser token issued by ORLEN
-- for a specific storefront domain, whereas PartnerID and PartnerKey remain secret.

ALTER TABLE public.shipping_settings
  ADD COLUMN IF NOT EXISTS orlen_environment text NOT NULL DEFAULT 'sandbox'
    CHECK (orlen_environment IN ('sandbox', 'production')),
  ADD COLUMN IF NOT EXISTS orlen_partner_id text,
  ADD COLUMN IF NOT EXISTS orlen_partner_key text,
  ADD COLUMN IF NOT EXISTS orlen_widget_token text,
  ADD COLUMN IF NOT EXISTS orlen_sender_name text,
  ADD COLUMN IF NOT EXISTS orlen_sender_street text,
  ADD COLUMN IF NOT EXISTS orlen_sender_postal_code text,
  ADD COLUMN IF NOT EXISTS orlen_sender_city text,
  ADD COLUMN IF NOT EXISTS orlen_sender_phone text,
  ADD COLUMN IF NOT EXISTS orlen_sender_email text;
...
ALTER TABLE public.orders
  ...
  CHECK (shipping_method IN ('inpost', 'orlen', 'courier'));
...
CREATE OR REPLACE FUNCTION public.create_order(
  ...
  _pickup_point_code text DEFAULT NULL
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF _shipping_method NOT IN ('inpost', 'orlen', 'courier') THEN RAISE EXCEPTION 'invalid_shipping_method'; END IF;

  IF _shipping_method IN ('inpost', 'orlen') THEN
    IF coalesce(length(trim(_pickup_point_name)), 0) = 0 THEN RAISE EXCEPTION 'pickup_point_required'; END IF;
    IF _shipping_method = 'orlen' AND coalesce(length(trim(_pickup_point_code)), 0) = 0 THEN RAISE EXCEPTION 'pickup_point_code_required'; END IF;
  ...
  ELSE
    ...
    _shipping_phone := _phone_clean; _pickup_point_name := NULL; _pickup_point_address := NULL; _pickup_point_city := NULL; _pickup_point_code := NULL;
  END IF;
  ...
  FOR _item IN SELECT * FROM jsonb_array_elements(_items) LOOP
    ...
    SELECT price_grosze, language_code, country_id INTO _price_grosze, _primary_language_code, _country_id FROM public.card_designs WHERE id = _design_id AND active = true AND price_grosze > 0;
    ...
    IF _secondary_language_code IS NOT NULL THEN
      ...
      SELECT language_name, front_thank_you_text INTO _secondary_language_name, _secondary_front_thank_you_text FROM public.card_language_templates WHERE country_id = _country_id AND language_code = _secondary_language_code;
  ...
  INSERT INTO public.orders(user_id, total_amount, pickup_point_name, pickup_point_address, pickup_point_city, pickup_point_code, shipping_cost, invoice_requested, company_name, company_nip, company_address, payment_method, shipping_method, shipping_name, shipping_address, shipping_postal_code, shipping_city, shipping_country, shipping_phone, customer_email)
  VALUES (_uid, _total, _pickup_point_name, _pickup_point_address, _pickup_point_city, _pickup_point_code, _shipping_cost, _invoice_requested, _company_name, _nip_clean, _company_address, _payment_method, _shipping_method, _shipping_name, _shipping_street, _shipping_postal_code, _shipping_city, CASE WHEN _shipping_method = 'courier' THEN 'Polska' ELSE NULL END, _shipping_phone, _customer_email)
  RETURNING id, order_number INTO _order_id, _order_number;

  FOR _item IN SELECT * FROM jsonb_array_elements(_items) LOOP
    _design_id := (_item->>'card_design_id')::uuid; _qty := (_item->>'quantity')::integer;
    SELECT price_grosze, language_code, country_id INTO _price_grosze, _primary_language_code, _country_id FROM public.card_designs WHERE id = _design_id;
    _secondary_language_code := nullif(trim(_item->>'secondary_language_code'), ''); _secondary_language_name := NULL; _secondary_front_thank_you_text := NULL;
    IF _secondary_language_code IS NOT NULL THEN SELECT language_name, front_thank_you_text INTO _secondary_language_name, _secondary_front_thank_you_text FROM public.card_language_templates WHERE country_id = _country_id AND language_code = _secondary_language_code; END IF;
    _unit_price := _price_grosze::numeric / 100.0;
    INSERT INTO public.order_items(order_id, card_design_id, quantity, unit_price, total_price, secondary_language_code, secondary_language_name, secondary_front_thank_you_text)
    VALUES (_order_id, _design_id, _qty, _unit_price, _qty * _unit_price, _secondary_language_code, _secondary_language_name, _secondary_front_thank_you_text);
  END LOOP;
...
REVOKE EXECUTE ON FUNCTION public.create_order(jsonb, text, text, text, numeric, boolean, text, text, text, text, text, text, text, text, text, text, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.create_order(jsonb, text, text, text, numeric, boolean, text, text, text, text, text, text, text, text, text, text, text) TO authenticated;
