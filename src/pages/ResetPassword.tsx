...
const ResetPassword = () => {
  ...
  useEffect(() => {
    // Check if session is present (Supabase sets session from hash/code automatically)
    ...
    try {
      ...
      if (error) {
        ...
        toast({
          ...
          description: error.message || "Wystąpił błąd podczas zmiany hasła. Spróbuj ponownie.",
          variant: "destructive",
        });
      } else {
        setIsSuccess(true);
        toast({
          ...
          description: "Możesz się teraz zalogować za pomocą nowego hasła.",
        });
        setTimeout(() => {
          navigate("/logowanie");
        }, 2500);
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <Helmet>
        <title>Ustaw nowe hasło — Podróżówka</title>
        <meta name="description" content="Ustaw nowe hasło do swojego konta Podróżówka." />
      </Helmet>

      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        <Link to="/logowanie" className="inline-flex items-center gap-2 text-muted-foreground hover:text-foreground transition-colors mb-6">
        ...
        <div className="bg-card rounded-2xl shadow-card p-8">
          <div className="text-center mb-8">
            ...
            <p className="text-muted-foreground text-sm">
              Wprowadź swoje nowe bezpieczne hasło do konta.
            </p>
          </div>
          ...
          {!sessionChecked ? (
            ...
            <div className="text-center py-6 space-y-4">
              ...
              <Button asChild className="w-full">
                <Link to="/odzyskiwanie-hasla">Przejdź do odzyskiwania hasła</Link>
              </Button>
            </div>
          ) : isSuccess ? (
            <div className="text-center py-6 space-y-4">
              ...
              <Button asChild className="w-full">
                <Link to="/logowanie">Zaloguj się nowym hasłem</Link>
              </Button>
            </div>
          ) : (
