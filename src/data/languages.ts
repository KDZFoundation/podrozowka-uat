export interface LanguageOption {
  code: string;
  name_pl: string;
  name_native: string;
  default_front_thank_you?: string;
  default_back_qr_label?: string;
}

export const WORLD_LANGUAGES: LanguageOption[] = [
  { code: "pl", name_pl: "Polski", name_native: "Polski", default_front_thank_you: "DZIĘKUJĘ, ŻE JESTEŚ CZĘŚCIĄ MOJEJ PODRÓŻY", default_back_qr_label: "Zeskanuj ten kod QR i śledź moje podróże" },
  { code: "en", name_pl: "Angielski", name_native: "English", default_front_thank_you: "THANK YOU FOR BEING PART OF MY JOURNEY", default_back_qr_label: "Scan this QR code and follow my travels" },
  { code: "en-au", name_pl: "Angielski (Australia)", name_native: "English (Australia)", default_front_thank_you: "THANK YOU FOR BEING PART OF MY JOURNEY", default_back_qr_label: "Scan this QR code and follow my travels" },
  { code: "de", name_pl: "Niemiecki", name_native: "Deutsch", default_front_thank_you: "DANKE, DASS DU TEIL MEINER REISE BIST", default_back_qr_label: "Scanne diesen QR-Code und begleite meine Reisen" },
  { code: "fr", name_pl: "Francuski", name_native: "Français", default_front_thank_you: "MERCI DE FAIRE PARTIE DE MON VOYAGE", default_back_qr_label: "Scanne ce code QR et suis mes voyages" },
  { code: "es", name_pl: "Hiszpański", name_native: "Español", default_front_thank_you: "GRACIAS POR SER PARTE DE MI VIAJE", default_back_qr_label: "Escanea este código QR y sigue mis viajes" },
  { code: "ca", name_pl: "Kataloński", name_native: "Català", default_front_thank_you: "GRÀCIES PER FORMAR PART DEL MEU VIATGE", default_back_qr_label: "Escaneja aquest codi QR i segueix els meus viatges" },
  { code: "gl", name_pl: "Galicyjski", name_native: "Galego", default_front_thank_you: "GRAZAS POR FORMAR PARTE DA MIÑA VIAXE", default_back_qr_label: "Escanea este código QR e segue as miñas viaxes" },
  { code: "eu", name_pl: "Baskijski", name_native: "Euskara", default_front_thank_you: "ESKERRIK ASKO NIRE BIDAIAREN PARTE IZATEAGATIK", default_back_qr_label: "Eskaneatu QR kode hau eta jarraitu nire bidaiak" },
  { code: "oc", name_pl: "Araneński", name_native: "Aranés", default_front_thank_you: "MERCÉS PER ÈSTER PART DETH MÈU VIATGE", default_back_qr_label: "Escanège aguest còdi QR e seguís es mèus viatges" },
  { code: "it", name_pl: "Włoski", name_native: "Italiano", default_front_thank_you: "GRAZIE PER FAR PARTE DEL MIO VIAGGIO", default_back_qr_label: "Scansiona questo codice QR e segui i miei viaggi" },
  { code: "uk", name_pl: "Ukraiński", name_native: "Українська", default_front_thank_you: "ДЯКУЮ, ЩО ТИ Є ЧАСТИНОЮ МОЄЇ ПОДОРОЖІ", default_back_qr_label: "Скануй цей QR-код і стеж за моїми подорожами" },
  { code: "cs", name_pl: "Czeski", name_native: "Čeština", default_front_thank_you: "DĚKUJI, ŽE JSI SOUČÁSTÍ MÉ CESTY", default_back_qr_label: "Naskenuj tento QR kód a sleduj mé cesty" },
  { code: "hr", name_pl: "Chorwacki", name_native: "Hrvatski", default_front_thank_you: "HVALA ŠTO SI DIO MOG PUTOVANJA", default_back_qr_label: "Skeniraj ovaj QR kod i prati moja putovanja" },
  { code: "el", name_pl: "Grecki", name_native: "Ελληνικά", default_front_thank_you: "ΕΥΧΑΡΙΣΤΩ ΠΟΥ ΕΙΣΑΙ ΜΕΡΟΣ ΤΟΥ ΤΑΞΙΔΙΟΥ ΜΟΥ", default_back_qr_label: "Σκάναρε αυτόν τον κωδικό QR και ακολούθησε τα ταξίδια μου" },
  { code: "hu", name_pl: "Węgierski", name_native: "Magyar", default_front_thank_you: "KÖSZÖNÖM, HOGY RÉSZE VAGY AZ UTAZÁSOMNAK", default_back_qr_label: "Szkenneld be ezt a QR-kódot, és kövesd az utazásaimat" },
  { code: "tr", name_pl: "Turecki", name_native: "Türkçe", default_front_thank_you: "YOLCULUĞUMUN BİR PARÇASI OLDUĞUN İÇİN TEŞEKKÜR EDERİM", default_back_qr_label: "Bu QR kodunu tara ve yolculuklarımı takip et" },
  { code: "th", name_pl: "Tajski", name_native: "ไทย", default_front_thank_you: "ขอบคุณที่เป็นส่วนหนึ่งของการเดินทางของฉัน", default_back_qr_label: "สแกนคิวอาร์โค้ดนี้และติดตามการเดินทางของฉัน" },
  { code: "zh", name_pl: "Chiński", name_native: "中文", default_front_thank_you: "感谢你成为我旅程的一部分", default_back_qr_label: "扫描此二维码，关注我的旅程" },
  { code: "ja", name_pl: "Japoński", name_native: "日本語", default_front_thank_you: "私の旅の一部になってくれてありがとう", default_back_qr_label: "このQRコードをスキャンして、私の旅を追ってください" },
  { code: "ar", name_pl: "Arabski", name_native: "العربية", default_front_thank_you: "شكرًا لكونك جزءًا من رحلتي", default_back_qr_label: "امسح رمز الاستجابة السريعة هذا وتابع رحلاتي" },
  { code: "pt", name_pl: "Portugalski", name_native: "Português", default_front_thank_you: "OBRIGADO POR FAZERES PARTE DA MINHA VIAGEM", default_back_qr_label: "Digitaliza este código QR e acompanha as minhas viagens" },
  { code: "nl", name_pl: "Holenderski", name_native: "Nederlands", default_front_thank_you: "BEDANKT DAT JE DEEL UITMAAKT VAN MIJN REIS", default_back_qr_label: "Scan deze QR-code en volg mijn reizen" },
  { code: "sv", name_pl: "Szwedzki", name_native: "Svenska", default_front_thank_you: "TACK FÖR ATT DU ÄR EN DEL AV MIN RESA", default_back_qr_label: "Skanna den här QR-koden och följ mina resor" },
  { code: "no", name_pl: "Norweski", name_native: "Norsk", default_front_thank_you: "TAKK FOR AT DU ER EN DEL AV REISEN MIN", default_back_qr_label: "Skann denne QR-koden og følg reisene mine" },
  { code: "da", name_pl: "Duński", name_native: "Dansk", default_front_thank_you: "TAK FORDI DU ER EN DEL AF MIN REJSE", default_back_qr_label: "Scan denne QR-kode og følg mine rejser" },
  { code: "fi", name_pl: "Fiński", name_native: "Suomi", default_front_thank_you: "KIITOS, ETTÄ OLET OSA MATKAANI", default_back_qr_label: "Skannaa tämä QR-koodi ja seuraa matkojani" },
  { code: "ro", name_pl: "Rumuński", name_native: "Română", default_front_thank_you: "ÎȚI MULȚUMESC CĂ FACI PARTE DIN CĂLĂTORIA MEA", default_back_qr_label: "Scanează acest cod QR și urmărește-mi călătoriile" },
  { code: "sk", name_pl: "Słowacki", name_native: "Slovenčina", default_front_thank_you: "ĎAKUJEM, ŽE SI SÚČASŤOU MOJEJ CESTY", default_back_qr_label: "Naskenuj tento QR kód a sleduj moje cesty" },
  { code: "sl", name_pl: "Słoweński", name_native: "Slovenščina", default_front_thank_you: "HVALA, KER SI DEL MOJEGA POTOVANJA", default_back_qr_label: "Skeniraj to QR-kodo in spremljaj moja potovanja" },
  { code: "bg", name_pl: "Bułgarski", name_native: "Български", default_front_thank_you: "БЛАГОДАРЯ, ЧЕ СИ ЧАСТ ОТ МОЕТО ПЪТУВАНЕ", default_back_qr_label: "Сканирай този QR код и следвай моите пътувания" },
  { code: "lt", name_pl: "Litewski", name_native: "Lietuvių", default_front_thank_you: "AČIŪ, KAD ESI MANO KELIONĖS DALIS", default_back_qr_label: "Nuskaityk šį QR kodą ir sek mano keliones" },
  { code: "lv", name_pl: "Łotewski", name_native: "Latviešu", default_front_thank_you: "PALDIES, KA ESI DAĻA NO MANA CEĻOJUMA", default_back_qr_label: "Skenē šo QR kodu un seko maniem ceļojumiem" },
  { code: "et", name_pl: "Estoński", name_native: "Eesti", default_front_thank_you: "AITÄH, ET OLED OSA MINU TEEKONNAST", default_back_qr_label: "Skanni see QR-kood ja jälgi minu reise" },
  { code: "vi", name_pl: "Wietnamski", name_native: "Tiếng Việt", default_front_thank_you: "CẢM ƠN BẠN ĐÃ LÀ MỘT PHẦN TRONG HÀNH TRÌNH CỦA TÔI", default_back_qr_label: "Quét mã QR này và theo dõi những chuyến đi của tôi" },
  { code: "hi", name_pl: "Hinduski", name_native: "हिन्दी", default_front_thank_you: "मेरी यात्रा का हिस्सा बनने के लिए धन्यवाद", default_back_qr_label: "इस QR कोड को स्कैन करें और मेरी यात्राओं का अनुसरण करें" },
  { code: "id", name_pl: "Indonezyjski", name_native: "Bahasa Indonesia", default_front_thank_you: "TERIMA KASIH SUDAH MENJADI BAGIAN DARI PERJALANANKU", default_back_qr_label: "Pindai kode QR ini dan ikuti perjalanan saya" },
  { code: "nl-be", name_pl: "Niderlandzki (flamandzki)", name_native: "Vlaams", default_front_thank_you: "BEDANKT DAT JE DEEL UITMAAKT VAN MIJN REIS", default_back_qr_label: "Scan deze QR-code en volg mijn reizen" },
  { code: "is", name_pl: "Islandzki", name_native: "Íslenska", default_front_thank_you: "TAKK FYRIR AÐ VERA HLUTI AF FERÐ MINNI", default_back_qr_label: "Skannaðu þennan QR-kóða og fylgstu með ferðum mínum" },
  { code: "sn", name_pl: "Szona", name_native: "chiShona", default_front_thank_you: "NDINOTENDA NEKUTI URI CHIKAMU CHERWENDO RWANGU", default_back_qr_label: "Skena QR code iyi utevere mafambiro angu" },
  { code: "nd", name_pl: "Ndebele (północny)", name_native: "isiNdebele", default_front_thank_you: "NGIYABONGA NGOKUBA YINGXENYE YOHAMBO LWAMI", default_back_qr_label: "Skena le khodi ye-QR ulandele uhambo lwami" },
  { code: "mt", name_pl: "Maltański", name_native: "Malti", default_front_thank_you: "GRAZZI TAL-FATT LI INT PARTI MILL-VJAĠĠ TIEGĦI", default_back_qr_label: "Skennja dan il-kodiċi QR u segwi l-vjaġġi tiegħi" },
  { code: "cnr", name_pl: "Czarnogórski", name_native: "Crnogorski", default_front_thank_you: "HVALA ŠTO SI DIO MOG PUTOVANJA", default_back_qr_label: "Skeniraj ovaj QR kod i prati moja putovanja" },
  { code: "af", name_pl: "Afrikaans", name_native: "Afrikaans", default_front_thank_you: "DANKIE DAT JY DEEL IS VAN MY REIS", default_back_qr_label: "Skandeer hierdie QR-kode en volg my reise" },
  { code: "zu", name_pl: "Zulu", name_native: "isiZulu", default_front_thank_you: "NGIYABONGA NGOKUBA UYINGXENYE YOHAMBO LWAMI", default_back_qr_label: "Skena le khodi ye-QR bese ulandela uhambo lwami" },
  { code: "xh", name_pl: "Xhosa", name_native: "isiXhosa", default_front_thank_you: "ENKOSI NGOKUBA YINXENYE YOHAMBO LWAM", default_back_qr_label: "Skena le khowudi ye-QR ulandele iihambo zam" },
  { code: "sw", name_pl: "Suahili", name_native: "Kiswahili", default_front_thank_you: "ASANTE KWA KUWA SEHEMU YA SAFARI YANGU", default_back_qr_label: "Changanua msimbo huu wa QR na ufuate safari zangu" },
  { code: "ga", name_pl: "Irlandzki (Gaeilge)", name_native: "Gaeilge", default_front_thank_you: "GO RAIBH MAITH AGAT A BHEITH I DO CHUID DE MO THURAS", default_back_qr_label: "Scan an cód QR seo agus lean mo thurais" },
  { code: "bem", name_pl: "Bemba", name_native: "Ichibemba", default_front_thank_you: "NATOTELA UKUBA ULI CIKAMU CA LUBENDO LWANDI", default_back_qr_label: "Scan QR code iyi no landa ulwendo lwandi" },
  { code: "ny", name_pl: "Nianja", name_native: "Chichewa / Chinyanja", default_front_thank_you: "ZIKOMO CHIFUKWA CHAKUTI NDINU GAWO LA ULENDO WANGA", default_back_qr_label: "Jambulani nambala iyi ya QR ndikutsatira maulendo anga" },
  { code: "toi", name_pl: "Tonga (Afryka)", name_native: "Chitonga", default_front_thank_you: "NDINOTENDA NEKUVA CHIKAMU CHEKUFAMBA KWANGU", default_back_qr_label: "Skena kodhi iyi ye-QR utevere mafambiro angu" },
  { code: "loz", name_pl: "Lozi", name_native: "Silozi", default_front_thank_you: "NI LITUMELO KWA KUBA ULI KARAPA YA LWIENDO LWANGU", default_back_qr_label: "Skena khoutu ye-QR ye, u latele maeto a ka" },
  { code: "si", name_pl: "Syngaleski (Sinhala)", name_native: "සිංහල", default_front_thank_you: "මගේ ගමනේ කොටසක් වීම ගැන ස්තුතියි", default_back_qr_label: "මෙම QR කේතය ස්කෑන් කර මගේ ගමන් අනුගමනය කරන්න" },
  { code: "ta", name_pl: "Tamilski", name_native: "தமிழ்", default_front_thank_you: "என் பயணத்தின் ஒரு பகுதியாக இருப்பதற்கு நன்றி", default_back_qr_label: "இந்த QR குறியீட்டை ஸ்கேன் செய்து என் பயணங்களைப் பின்தொடருங்கள்" },
  { code: "dv", name_pl: "Malediwski (Dhivehi)", name_native: "ދިވެހިބަސް", default_front_thank_you: "ދިޔައްޔެ މަގުގެ ބައެއް ވީތީ ޝުކުރިއްޔާ", default_back_qr_label: "މި QR ކޯޑު ސްކޭން ކޮށް މަގުގައި ދިޔުމަށް ތިބާ ބަލާ" },
  { code: "ka", name_pl: "Gruziński", name_native: "ქართული", default_front_thank_you: "გმადლობთ, რომ ჩემი მოგზაურობის ნაწილი ხართ", default_back_qr_label: "დაასკანერეთ ეს QR კოდი და თვალი ადევნეთ ჩემს მოგზაურობებს" },
  { code: "mfe", name_pl: "Kreolski maurytyjski", name_native: "Kreol Morisien", default_front_thank_you: "Mersi pou fer parti mo vwayaz", default_back_qr_label: "Eskane sa kòd QR-la ek swiv mo bann vwayaz" },
  { code: "bn", name_pl: "Bengalski", name_native: "বাংলা", default_front_thank_you: "আমার ভ্রমণের অংশ হওয়ার জন্য ধন্যবাদ", default_back_qr_label: "এই QR কোডটি স্ক্যান করুন এবং আমার ভ্রমণ অনুসরণ করুন" },
  { code: "te", name_pl: "Telugu", name_native: "తెలుగు", default_front_thank_you: "నా ప్రయాణంలో భాగమైనందుకు ధన్యవాదాలు", default_back_qr_label: "ఈ QR కోడ్‌ను స్కాన్ చేసి నా ప్రయాణాలను అనుసరించండి" },
  { code: "tzm", name_pl: "Tamazight", name_native: "ⵜⴰⵎⴰⵣⵉⵖⵜ", default_front_thank_you: "ⵜⴰⵏⵎⵎⵉⵔⵜ ⵉⵎⵉ ⴷ ⵜⴳⴳⵉⴷ ⴰⵎⵓⵔ ⵏ ⵜⵉⴽⵍⵉ ⵉⵏⵓ", default_back_qr_label: "ⵙⴽⴰⵏ ⴰⵙ ⵜⴰⵏⴳⴰⵍⵜⴰ ⵏ QR ⵙⴰⴽⵉⵏ ⴷⴼⴻⵔ ⵜⵉⴽⵍⵉⵡⵉⵏ ⵉⵏⵓ" },
  { code: "roh", name_pl: "Retoromański", name_native: "Rumantsch", default_front_thank_you: "GRAZIA CHE TI ES PART DA MI VIADI", default_back_qr_label: "Scannescha quest code QR e suonda mes viadis" },
  { code: "fr-ca", name_pl: "Francuski (Kanada)", name_native: "Français canadien", default_front_thank_you: "MERCI DE FAIRE PARTIE DE MON VOYAGE", default_back_qr_label: "Scannez ce code QR et suivez mes voyages" },
  { code: "gd", name_pl: "Szkocki gaelicki", name_native: "Gàidhlig", default_front_thank_you: "TAPADH LEAT A BHITH NAD PÀIRT DEN TURAS AGAM", default_back_qr_label: "Sgan an còd QR seo agus lean mo thursan" },
  { code: "cy", name_pl: "Walijski", name_native: "Cymraeg", default_front_thank_you: "DIOLCH AM FOD YN RHAN O FY SIWRNAI", default_back_qr_label: "Sganiwch y cod QR hwn a dilynwch fy nheithiau" },
  { code: "ty", name_pl: "Tahitański", name_native: "Reo Tahiti", default_front_thank_you: "MAURUURU I TEI TE HOƆ E TAU I TAU TERE", default_back_qr_label: "Tāpa'o i teie numera QR e pe'e i ta'u tere" },
  { code: "yue", name_pl: "Kantoński", name_native: "粵語", default_front_thank_you: "多謝你成為我旅程嘅一部分", default_back_qr_label: "掃描呢個 QR 碼，跟住我嘅旅程" },
  { code: "fa", name_pl: "Perski", name_native: "فارسی", default_front_thank_you: "ممنون که بخشی از سفر من هستی", default_back_qr_label: "این کد QR را اسکن کنید و سفرهای من را دنبال کنید" },
  { code: "hy", name_pl: "Armeński", name_native: "Հայերեն", default_front_thank_you: "ՇՆՈՐՀԱԿԱԼՈՒԹՅՈՒՆ, ՈՐ ԻՄ ՃԱՆԱՊԱՐՀՈՐԴՈՒԹՅԱՆ ՄԱՍՆ ԵՍ", default_back_qr_label: "Սկանավորեք այս QR կոդը և հետևեք իմ ճանապարհորդություններին" },
  { code: "sq", name_pl: "Albański", name_native: "Shqip", default_front_thank_you: "FALEMINDERIT QË JE PJESË E UDHËTIMIT TIM", default_back_qr_label: "Skano këtë kod QR dhe ndiq udhëtimet e mia" },
  { code: "sr", name_pl: "Serbski", name_native: "Српски", default_front_thank_you: "ХВАЛА ШТО СИ ДЕО МОГ ПУТОВАЊА", default_back_qr_label: "Скенирај овај QR код и прати моја путовања" },
  { code: "km", name_pl: "Khmerski", name_native: "ខ្មែរ", default_front_thank_you: "សូមអរគុណដែលអ្នកជាផ្នែកមួយនៃដំណើររបស់ខ្ញុំ", default_back_qr_label: "ស្កេនកូដ QR នេះ ហើយតាមដានដំណើររបស់ខ្ញុំ" },
  { code: "lb", name_pl: "Luksemburski", name_native: "Lëtzebuergesch", default_front_thank_you: "MERCI, DATT S DU EN DEEL VU MENGER REES BASS", default_back_qr_label: "SCAN DËSE QR-CODE A FOLLEG MENG REESEN" },
  { code: "lo", name_pl: "Laotański", name_native: "ລາວ", default_front_thank_you: "ຂອບໃຈທີ່ເປັນສ່ວນໜຶ່ງຂອງການເດີນທາງຂອງຂ້ອຍ", default_back_qr_label: "ສະແກນລະຫັດ QR ນີ້ ແລະຕິດຕາມການເດີນທາງຂອງຂ້ອຍ" },
  { code: "bs", name_pl: "Bośniacki", name_native: "Bosanski", default_front_thank_you: "HVALA ŠTO SI DIO MOG PUTOVANJA", default_back_qr_label: "SKENIRAJ OVAJ QR KOD I PRATI MOJA PUTOVANJA" },
  { code: "az", name_pl: "Azerbejdżański", name_native: "Azərbaycanca", default_front_thank_you: "SƏYAHƏTİMİN BİR HİSSƏSİ OLDUĞUN ÜÇÜN TƏŞƏKKÜR EDİRƏM", default_back_qr_label: "BU QR KODU SKAN EDİN VƏ SƏYAHƏTLƏRİMİ İZLƏYİN" },
  { code: "ru", name_pl: "Rosyjski", name_native: "Русский", default_front_thank_you: "СПАСИБО, ЧТО ТЫ ЧАСТЬ МОЕГО ПУТЕШЕСТВИЯ", default_back_qr_label: "ОТСКАНИРУЙТЕ ЭТОТ QR-КОД И СЛЕДИТЕ ЗА МОИМИ ПУТЕШЕСТВИЯМИ" },
  { code: "mk", name_pl: "Macedoński", name_native: "Македонски", default_front_thank_you: "ТИ БЛАГОДАРАМ ШТО СИ ДЕЛ ОД МОЕТО ПАТУВАЊЕ", default_back_qr_label: "СКЕНИРАЈ ГО ОВОЈ QR-КОД И СЛЕДИ ГИ МОИТЕ ПАТУВАЊА" },
  { code: "he", name_pl: "Hebrajski", name_native: "עברית", default_front_thank_you: "תודה שאתה חלק מהמסע שלי", default_back_qr_label: "סרקו את קוד ה‑QR הזה ועקבו אחר המסעות שלי" },
  { code: "am", name_pl: "Amharski", name_native: "አማርኛ", default_front_thank_you: "የጉዞዬ አካል ስለሆንክ እናመሰግናለን", default_back_qr_label: "ይህን QR ኮድ ይቃኙ እና ጉዞዎቼን ይከተሉ" },
  { code: "om", name_pl: "Oromski", name_native: "Afaan Oromoo", default_front_thank_you: "IMALA KOO KEESSA QOODDAA TA'UU KEETIIF GALATOOMI", default_back_qr_label: "KODII QR KANA SKAAN GODHIITII IMALA KOO HORDOFI" },
  { code: "kk", name_pl: "Kazachski", name_native: "Қазақша", default_front_thank_you: "САПАРЫМНЫҢ БІР БӨЛІГІ БОЛҒАНЫҢ ҮШІН РАҚМЕТ", default_back_qr_label: "ОСЫ QR КОДЫН СКАНЕРЛЕП, САПАРЛАРЫМДЫ ҚАДАҒАЛА" },
  { code: "ko", name_pl: "Koreański", name_native: "한국어", default_front_thank_you: "내 여행의 일부가 되어 주셔서 감사합니다", default_back_qr_label: "이 QR 코드를 스캔하고 내 여행을 따라오세요" },
  { code: "uz", name_pl: "Uzbecki", name_native: "Oʻzbekcha", default_front_thank_you: "SAYOHATIMNING BIR QISMI BO'LGANINGIZ UCHUN RAHMAT", default_back_qr_label: "USHBU QR KODNI SKANERLANG VA SAYOHATLARIMNI KUZATING" },
  { code: "wo", name_pl: "Wolof", name_native: "Wolof", default_front_thank_you: "JËREJËF CI SA BOKK CI SAMA TUKKI", default_back_qr_label: "SKANNEEL KOODU QR BII TE TOPP SAMA TUKKI YI" },
  { code: "mnk", name_pl: "Mandinka", name_native: "Mandinka", default_front_thank_you: "ABARAKA I YE N'NA TAARIKOO KAFUŊO LA", default_back_qr_label: "SCAN QR KODU NIN NAŊ N'NA TAARIKOO LU KANU" },
  { code: "be", name_pl: "Białoruski", name_native: "Беларуская", default_front_thank_you: "ДЗЯКУЙ, ШТО ТЫ ЧАСТКА МАЁЙ ВАНДРОЎКІ", default_back_qr_label: "АДСКАНУЙЦЕ ГЭТЫ QR-КОД І САЧЫЦЕ ЗА МАІМІ ПАДАРОЖЖАМІ" },
  { code: "qu", name_pl: "Keczua", name_native: "Runa Simi", default_front_thank_you: "SULPAYKI ÑUQAPA PURIYNIYPI PARTE KASQAYKIMANTA", default_back_qr_label: "KAY QR CODIGOTA QHAWAY HINASPA PURIYNIYKUNATA QATIPAY" },
  { code: "fil", name_pl: "Filipiński", name_native: "Filipino", default_front_thank_you: "SALAMAT SA PAGIGING BAHAGI NG AKING PAGLALAKBAY", default_back_qr_label: "I-SCAN ANG QR CODE NA ITO AT SUNDAN ANG AKING MGA PAGLALAKBAY" },
  { code: "ms", name_pl: "Malajski", name_native: "Bahasa Melayu", default_front_thank_you: "TERIMA KASIH KERANA MENJADI SEBAHAGIAN DARIPADA PERJALANAN SAYA", default_back_qr_label: "IMBAS KOD QR INI DAN IKUTI PERJALANAN SAYA" },
];
