...
export default {
  async fetch(request: Request) {
    ...
    try {
      ...
      if (!amount || !paymentId || !orderNumber || !status || !secure || !incomingHash || !secret || !password) return new Response("bad request", { status: 400 });
      if (!safeEquals(secure, secret)) return new Response("invalid secret", { status: 400 });
      const expected = sha256(`${password};${amount};${paymentId};${orderNumber};${status};${secure};${secret}`);
